...
def gram_rbf(X, Y, gammas, save="", test=False):
    n, d = X.shape
    m = Y.shape[0]
    assert d == Y.shape[1]
    vect_diff = Y.unsqueeze(1) - X
    assert (m, n, d) == vect_diff.shape
    vect_norm = (vect_diff ** 2).sum(dim=-1)  # 计算向量差的平方和
    assert (m, n) == vect_norm.shape
    K_val = torch.exp(-torch.einsum('g, nm->gnm', gammas, vect_norm))  # 计算RBF核矩阵
    assert (len(gammas), m, n) == K_val.shape
    if test:
        if save:
            torch.save(K_val, save + 'K_test.pt')
    ...
    vect_norm = (vect_diff ** 2).sum(dim=-1)  # 计算向量差的平方和
    assert (n, n) == vect_norm.shape
    K_train = torch.exp(-torch.einsum('g, nm->gnm', gammas, vect_norm))  # 计算RBF核矩阵
    assert (len(gammas), n, n) == K_train.shape
    if save:
        torch.save(K_train, save + 'K_train.pt')
        torch.save(K_val, save + 'K_val.pt')
