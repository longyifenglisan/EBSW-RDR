...
def train_multiple_kernels(Ks, K_tests, y_train, y_test, hp_clf, hp_kernel, subsample=1, save=""):
    ...
    assert Ks.dim() == 3
    n_Ks = len(Ks) #Ks中核矩阵的个数
    assert n_Ks == len(K_tests)
    results = {}
    dict_best = {}
    #print("Starting training...\n")
    # 新增两个列表用于存储参数组合
    kernel_param_combinations = []
    classifier_param_combinations = []
    ...
    for s in range(subsample):
        ...
        max_acc = 0
        idx = (0, 0)
        #遍历每一种核函数参数与分类器超参数的组合。#####用grid排列组合并行运算（对标matlab组合）。
        for j in range(n_Ks):#范围缩小，先尝试，再整论跑。
            kernel_param_combinations.append(hp_kernel[j])
            K, Ktest = K_train_s[j].to(device), K_test_s[j].to(device)
            for k in range(len(hp_clf)):
                classifier_param_combinations.append(hp_clf[k])
                clf = KRR(hp_clf[k]) #hp_clf参数矩阵中第K个超参数的值
    ...
    if save:
        ...
        #以二进制写入模式（wb）打开一个名为 save + 'full_results.pickle' 的文件
        with open(save + 'full_results.pickle', 'wb') as f:
            pickle.dump(results, f)#使用 pickle 模块将 results 字典对象序列化并保存到打开的文件中
